# Postman-коллекция для проверки Organization API

Файл `organization_api.postman_collection.json` содержит postman-коллекцию - набор заранее подготовленных запросов для проверки работы API organization_api проекта.
...
Загрузите файл `organization_api.postman_collection.json` в Postman.
...
При выполнении скрипта будут удалены все departmenty и employees, созданные при предыдущем запуске коллекции (при условии корректной настройки параметров `on_delete` в моделях проекта).

При сбое очистки базы данных, используйте резервную копию файла `db.sqlite3`: замените текущий файл базы данных на эту копию.
А можно создать базу данных заново (как описано в п.3 раздела _Подготовка Django-проекта к запуску коллекции_).

## Структура коллекции:

Коллекция содержит следующие группы тестов:

- **Department // Create** - Создание нового departmenta и departmenta с parent'ом
- **Department // Read** - Получение departmenta с различными параметрами (depth, include_employees)
- **Department // Update** - Обновление данных departmenta (имя, parent)
- **Department // Delete** - Удаление departmenta с режимами cascade и reassign
- **Employee // Create** - Создание сотрудников в departmente
- **Employee // Verify in Department** - Проверка что сотрудники были добавлены в department
